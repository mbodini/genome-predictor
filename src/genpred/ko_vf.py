...
def _get_out_path(cfg: DictConfig) -> Path:
    data = cfg.dataset.name
    vect = cfg.dataset.vectorizer
    vocab_size = cfg.dataset.vocab_size
    out_dir = genpred.EXPS_ROOT / "knockout"
    name = "virulence" if cfg.control is False else "virulence-control"
...
def main(cfg: DictConfig) -> None:
    ...
    genes = load_genes_list("virulence_factors.txt")
    if not (out_dir / "predictions.csv").exists():
        data = Parallel(verbose=1, n_jobs=-1)(
            delayed(_helper)(vectorizer, model, out_dir, row, genes, cfg.control) for row in preds.itertuples()
        )
